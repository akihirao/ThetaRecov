...
def calc_inbreed_light(vcf_path, output_csv = "inbreed.csv", coverage = 3):
    ...
    for i, j in selected_combinations:
        ...
        #diff_among_ij = np.sum(abs_diff_among * valid_mask, axis = 2) # (2,2)各ペアの総和
        #diff_among_ij = np.sum(diff_among_ij)
        #count_among_ij = np.sum(valid_mask)

        #print(diff_among_ij)
        #print(count_among_ij)

        #diff_among += diff_among_ij
        #count_among += count_among_ij
...
def calc_inbreed_boot(vcf_path, output_csv = "inbreed.csv", coverage = 3, num_boots = 100):
    ...
    for k in range(num_boots):
        ...
        results[k, 0] = pi_overall
        results[k, 1] = pi_within
        results[k, 2] = pi_among
        results[k, 3] = homo_deviance
